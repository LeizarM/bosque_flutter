...
class _InfoEmpleadoScreenState extends ConsumerState<InfoEmpleadoScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool canSeeReport =
        _habilitarEdicion ||
        (currentUser != null &&
            _reportAllowedEmployees.contains(currentUser.codEmpleado));
    ref.listen<int>(warningCounterProvider, (prev, next) async {
      ...
      if (user != null && user.codEmpleado == widget.codEmpleado) {
        if ((prev == null || prev == 0) && next == 1) {
          ...
          ref.read(
            registrarBloqueoUsuarioProvider({
    ...
    return isDesktop
        ? _buildDesktopLayout(canSeeReport)
        : _buildMobileLayout(canSeeReport);
  }
  ...
  Widget _buildDesktopLayout(canSeeReport) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Información del Empleado')),
      body:
          codEmpleadoUsuario == null
              ? const Center(child: CircularProgressIndicator())
              : ref
                  .watch(
                    empleadoXJerarquiaProvider((
                      codEmpleadoUsuario,
                      widget.codEmpleado,
                    )),
                  )
                            ...
                            child: Row(
                              children: [
                                    ...
                                    onPressed: () async {
                                      ...
                                      try {
                                        ...
                                        await Printing.layoutPdf(
                                          ...
                                          name:
                                              'RptFichaTrabajador', // ← nombre corregido
                                        );
                                      } catch (e) {
                                ...
                                IconButton(
                                  ...
                                  onPressed: () {
                                    ref.invalidate(
                                      empObtenerDatosEmpleados(
                                        widget.codEmpleado,
                                      ),
                                    );
                                    ref.invalidate(
                                      obtenerPersonaProvider(codPersona),
                                    );
                                    ref.invalidate(
                                      telefonoProvider(codPersona),
                                    );
                                    ...
                                    ref.invalidate(
                                      relacionLaboralProvider(
                                        widget.codEmpleado,
                                      ),
                                    );
                                    ref
                                        .read(imageVersionProvider.notifier)
                                        .state++;
                                    ref.invalidate(
                                      todosLosDocumentosProvider(
                                        widget.codEmpleado,
                                      ),
                                    );
                                    ref.invalidate(
                                                ...
                                                if (user != null) {
                                                  ...
                                                  if (context.mounted) {
                                                    ScaffoldMessenger.of(
                                                      ...
                                                      const SnackBar(
                                                        content: Text(
                                                          'No se encontró el usuario para este empleado',
                                                        ),
                                                        backgroundColor:
                                                            Colors.red,
                                                      ),
                      ...
                      builder: (context, ref, _) {
                        ...
                        return usersAsync.when(
                          data: (users) {
                            ...
                            try {
                              user = users.firstWhere(
                                (u) => u.codEmpleado == widget.codEmpleado,
                              );
                            } catch (e) {
                              user = null;
                            }
                              ...
                              data: (usuarioBloqueado) {
                                ...
                                return CronometroBloqueo(
                                  ...
                                  onFinalizado: () async {
                                    ...
                                    ref.invalidate(
                                      usuarioBloqueadoProvider(codUsuario),
                                    );
                                  },
                                );
                              },
      ...
      case 1:
        return _buildSection(
          child: FormacionSecccion(
            ...
            onToggleSeccion: toggleSeccion,
            onUpdateOperation:
                (op) => setState(() => selectedOperation['formacion'] = op),
      ...
      case 2:
        return _buildSection(
          child: ExperienciaLaboralSeccion(
            ...
            onUpdateOperation:
                (op) => setState(
                  () => selectedOperation['experienciaLaboral'] = op,
                ),
        ...
        actions: [
          if (canSeeReport)
            IconButton(
              ...
              onPressed: () async {
                ...
                try {
                  final pdfBytes = await ref.read(
                    jasperPdfProvider(widget.codEmpleado).future,
                  );
                  await Printing.layoutPdf(
                    onLayout: (format) async => pdfBytes,
          ...
          IconButton(
            ...
            onPressed: () {
              ...
              ref.invalidate(experienciaLaboralProvider(widget.codEmpleado));
              ref.invalidate(
                obtenerGaranteReferenciaProvider(widget.codEmpleado),
              );
              ref.invalidate(relacionLaboralProvider(widget.codEmpleado));
    ...
    return const SizedBox.shrink();
          ...
          FutureBuilder<String>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasData &&
                  snapshot.data!.toUpperCase() == 'ROLE_ADM') {
                return IconButton(
                  icon: const Icon(Icons.warning_amber, color: Colors.orange),
        ...
        bottom: PreferredSize(
          ...
          child: ref
              .watch(empObtenerDatosEmpleados(widget.codEmpleado))
              .when(
                data: (codPersona) {
                  _ultimoCodPersona = codPersona;
              ...
              : ref
                  ...
                  .when(
                    data: (empleado) {
                      ...
                      Future.microtask(() {
                        ref.refresh(
                          obtenerPersonaProvider(empleado.codPersona),
                        );
                      ...
                      return RefreshIndicator(
                        onRefresh: () async {
                          ref.invalidate(
                            empObtenerDatosEmpleados(widget.codEmpleado),
                          );
                          ref.invalidate(
                            obtenerPersonaProvider(empleado.codPersona),
                          );
                        ...
                        child: ListView(
                          ...
                          children: [
                            ...
                            if (_currentPage == 1)
                              _buildContactPage(empleado.codPersona),
        ...
        children: [
          ...
          _buildSection(
            child: PersonaSection(
              ...
              onToggleSeccion: toggleSeccion,
              onUpdateOperation:
                  (op) => setState(() => selectedOperation['persona'] = op),
              onEditar: () => activarEdicion('persona'),
              //ocultarCamposSensibles: !esAutoConsulta,
            ),
          ),
        ...
        children: [
          _buildSection(
            child: TelefonoSection(
              ...
              onToggleSeccion: toggleSeccion,
              onUpdateOperation:
                  (op) => setState(() => selectedOperation['telefono'] = op),
              onEditar: () => activarEdicion('telefono'),
              onAgregar: () => (),
              onEliminar: () => (),
            ),
          ...
          _buildSection(
            child: EmailSeccion(
              ...
              onToggleSeccion: toggleSeccion,
              onUpdateOperation:
                  (op) => setState(() => selectedOperation['email'] = op),
              onEditar: () => activarEdicion('email'),
              onAgregar: () => (),
              onEliminar: () => (),
            ),
