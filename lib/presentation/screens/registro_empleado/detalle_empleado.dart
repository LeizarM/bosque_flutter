...
class DetalleEmpleado extends ConsumerStatefulWidget {
  ...
  const DetalleEmpleado({Key? key, required this.codEmpleado}) : super(key: key);
...
class _DetalleEmpleadoState extends ConsumerState<DetalleEmpleado> {
  ...
  Widget build(BuildContext context) {
    final empleadoAsync = ref.watch(empObtenerDatosEmpleado(widget.codEmpleado));
      ...
      data: (empleado) {
        return DefaultTabController(
          ...
          child: Scaffold(
            ...
            appBar: AppBar(
              title: Text('${empleado.persona.nombres} ${empleado.persona.apPaterno}'),
              bottom: TabBar(
                ...
                tabs: const [
                  ...
                  Tab(text: 'Seguros', icon: Icon(Icons.medical_services_outlined)),
                ],
              ),
            ),
            ...
            Expanded(
              child: TabBarView(
                children: [
                  ...
                  _tabWrap(DetalleInformacionLaboral(codEmpleado: empleado.codEmpleado)),
                  _tabWrap(Column(children: [
                    DetalleEducacion(codEmpleado: empleado.codEmpleado, mode: mode),
                    SizedBox(height: context.spacing), // Usar context.spacing
                    DetalleFormacion(codEmpleado: empleado.codEmpleado, mode: mode),
                  ])),
                  _tabWrap(DetalleExperienciaLaboral(codEmpleado: empleado.codEmpleado, mode: mode)),
                  _tabWrap(DetalleInformacionBancaria(codEmpleado: empleado.codEmpleado, mode: mode)),
                  _tabWrap(DetalleAfiliacionSeguro(codEmpleado: empleado.codEmpleado)),
                ],
              ),
            ),
          ],
    ...
    return SingleChildScrollView(
      ...
      child: LayoutBuilder(builder: (context, constraints) {
        ...
        final columnLeft = Column(
          ...
          children: [
            _buildCardSimple(SeccionFotoEmpleado(codEmpleado: empleado.codEmpleado)),
            SizedBox(height: context.spacing),
            DetalleDocumentosEmpleado(codEmpleado: empleado.codEmpleado),
            SizedBox(height: context.spacing),
            _buildCardSimple(Column(
              children: [
        ...
        final columnRight = Column(
          ...
          children: [
            ...
            SizedBox(height: context.spacing),
            _buildCardSimple(DetalleLicenciaConducir(codPersona: empleado.codPersona)),
          ],
        );
        ...
        if (wide) {
          ...
          return Column(
            ...
            children: [
              ...
              _buildCardSimple(DetallePersona(persona: persona, mode: mode)),
              SizedBox(height: context.spacing),
              _buildCardSimple(DetalleTelefono(codPersona: empleado.codPersona, mode: mode)),
              SizedBox(height: context.spacing),
              _buildCardSimple(DetalleEmail(codPersona: empleado.codPersona, mode: mode)),
              SizedBox(height: context.spacing),
              _buildCardSimple(DetalleLicenciaConducir(codPersona: empleado.codPersona)),
            ],
          );
  ...
  Widget _buildHeaderCompacto(EmpleadoEntity empleado, dynamic persona) {
    return Container(
      padding: EdgeInsets.all(context.smallSpacing), // Usar context.smallSpacing
      ...
      child: Row(
        children: [
          Icon(Icons.badge, color: Theme.of(context).primaryColor, size: context.iconSize), // Usar Theme.of(context).primaryColor y context.iconSize
          SizedBox(width: context.spacing), // Usar context.spacing
          Text('ID: ${empleado.codEmpleado}', style: context.bodyStyle.copyWith(fontWeight: FontWeight.bold)), // Usar context.bodyStyle
          const Spacer(),
          Text('${empleado.persona.datoPersona}', style: context.bodyLightStyle), // Usar context.bodyLightStyle
        ],
      ),
    );
  }
  ...
  Widget _buildCardSimple(Widget child) {
    return Card(
      ...
      child: Padding(padding: EdgeInsets.all(context.spacing), child: child), // Usar context.spacing
    );
  }

  Widget _tabWrap(Widget child) => SingleChildScrollView(padding: EdgeInsets.all(context.spacing), child: child); // Usar context.spacing
}