...
class _RegistroEmpleadoState extends ConsumerState<RegistroEmpleado> {
  ...
  static const String _prefsSelectedPersonaCodKey = 'registroEmpleado_selectedPersonaCod';
  static const String _prefsNewPersonaDataKey = 'registroEmpleado_newPersonaData';
  ...
  static const String _prefsRelacionLaboralKey = 'registroEmpleado_relacion_laboral';
  static const String _prefsRegistroFuncionesKey = 'registroEmpleado_registro_funciones';
  static const String _prefsCuentasBancariasKey = 'registroEmpleado_cuentas_bancarias';
...
Future<void> _saveProgressToLocal() async {
  ...
  if (persona.codPersona != 0) {
    ...
    await prefs.setString(_prefsNewPersonaDataKey, personaModelToJson(PersonaModel.fromEntity(persona)));
    await prefs.remove(_prefsSelectedPersonaCodKey);
  }
  ...
  try {
    ...
    if (educaciones.isNotEmpty) {
      final educacionJson = jsonEncode(
        educaciones.map((e) => EducacionModel.fromEntity(e).toJson()).toList()
      );
    ...
    if (formaciones.isNotEmpty) {
      final formacionJson = jsonEncode(
        formaciones.map((f) => FormacionModel.fromEntity(f).toJson()).toList()
      );
    ...
    if (experiencias.isNotEmpty) {
      final experienciaJson = jsonEncode(
        experiencias.map((e) => ExperienciaLaboralModel.fromEntity(e).toJson()).toList()
      );
    ...
    if (relaciones.isNotEmpty) {
      final relacionJson = jsonEncode(
        relaciones.map((r) => RelacionLaboralModel.fromEntity(r).toJson()).toList()
      );
    ...
    if (registroFunciones.isNotEmpty) {
      final cargosSerializados = registroFunciones.map((cargo) {
        return {
          'codCargoSucursal': cargo['codCargoSucursal'],
          'codCargoSucPlanilla': cargo['codCargoSucPlanilla'],
          'fechaInicio': (cargo['fechaInicio'] as DateTime).toIso8601String(),
          'cargoSucursal': cargo['cargoSucursal'] is CargoSucursalEntity
              ? CargoSucursalModel.fromEntity(cargo['cargoSucursal']).toJson()
              : cargo['cargoSucursal'],
          'cargoSucursalPlanilla': cargo['cargoSucursalPlanilla'] is CargoSucursalEntity
              ? CargoSucursalModel.fromEntity(cargo['cargoSucursalPlanilla']).toJson()
    ...
    if (cuentas.isNotEmpty) {
      final cuentasJson = jsonEncode(
        cuentas.map((c) => NroCuentaBancariaModel.fromEntity(c).toJson()).toList()
      );
...
Future<void> _loadProgressFromLocal() async {
  ...
  if (savedCod != null && savedCod > 0) {
    try {
      ...
      _showErrorSnackBar('No se pudo restaurar la persona. Selecciona de nuevo.');
      _limpiarProvidersTemporales();
  ...
  if (savedJson != null && savedJson.isNotEmpty) {
    try {
      ...
      _showErrorSnackBar('No se pudieron restaurar los datos. Comienza de nuevo.');
      _limpiarProvidersTemporales();
      setState(() => _currentStep = 0);
    }
  }
...
Future<void> _loadTemporaryProviders(SharedPreferences prefs) async {
  try {
    ...
    if (telefonosJson != null && telefonosJson.isNotEmpty) {
      try {
        ...
        final telefonos = decoded
            .map((e) => TelefonoModel.fromJson(Map<String, dynamic>.from(e as Map))
                .toEntity())
            .toList();
    ...
    if (emailsJson != null && emailsJson.isNotEmpty) {
      try {
        ...
        final emails = decoded
            .map((e) => EmailModel.fromJson(Map<String, dynamic>.from(e as Map))
                .toEntity())
            .toList();
    ...
    if (educacionJson != null && educacionJson.isNotEmpty) {
      try {
        ...
        final educaciones = decoded
            .map((e) => EducacionModel.fromJson(Map<String, dynamic>.from(e as Map))
                .toEntity())
            .toList();
    ...
    if (formacionJson != null && formacionJson.isNotEmpty) {
      try {
        ...
        final formaciones = decoded
            .map((e) => FormacionModel.fromJson(Map<String, dynamic>.from(e as Map))
                .toEntity())
            .toList();
    ...
    if (experienciaJson != null && experienciaJson.isNotEmpty) {
      try {
        ...
        final experiencias = decoded
            .map((e) => ExperienciaLaboralModel.fromJson(Map<String, dynamic>.from(e as Map))
                .toEntity())
            .toList();
    ...
    if (relacionJson != null && relacionJson.isNotEmpty) {
      try {
        ...
        final relaciones = decoded
            .map((e) => RelacionLaboralModel.fromJson(Map<String, dynamic>.from(e as Map))
                .toEntity())
            .toList();
    ...
    if (registroFuncionesJson != null && registroFuncionesJson.isNotEmpty) {
      try {
        ...
        final rawList = decoded.map((e) => Map<String, dynamic>.from(e as Map)).toList();

        // Reconstruir los campos que la UI espera como entidades/DateTime
        final List<Map<String, dynamic>> cargosConverted = rawList.map((raw) {
          final Map<String, dynamic> item = Map<String, dynamic>.from(raw);

          // cargoSucursal: Map → Entity
          final csRaw = item['cargoSucursal'];
          if (csRaw is Map) {
            item['cargoSucursal'] = CargoSucursalModel.fromJson(Map<String, dynamic>.from(csRaw)).toEntity();
          }

          // cargoSucursalPlanilla: Map → Entity
          final csPlanRaw = item['cargoSucursalPlanilla'];
          if (csPlanRaw is Map) {
            item['cargoSucursalPlanilla'] = CargoSucursalModel.fromJson(Map<String, dynamic>.from(csPlanRaw)).toEntity();
          }

          // fechaInicio: String ISO → DateTime
          final fechaRaw = item['fechaInicio'];
          if (fechaRaw is String) {
            item['fechaInicio'] = DateTime.tryParse(fechaRaw) ?? DateTime.now();
          }

          return item;
        }).toList();

        ref.read(tempRegistroFuncionesListProvider.notifier).state = cargosConverted;
        ...
        if (cargosConverted.isNotEmpty) {
          ...
          final cargoInterno = primerCargo['cargoSucursal'] as CargoSucursalEntity?;
          final cargoPlanilla = primerCargo['cargoSucursalPlanilla'] as CargoSucursalEntity?;
    ...
    if (cuentasJson != null && cuentasJson.isNotEmpty) {
      try {
        ...
        final cuentas = decoded
            .map((e) => NroCuentaBancariaModel.fromJson(Map<String, dynamic>.from(e as Map))
                .toEntity())
            .toList();
  ...
  Widget _buildSelectPersonStep() {
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          constraints: BoxConstraints(maxWidth: context.isMobile ? double.infinity : 600),
          child: TextField(
        ...
        SizedBox(
          ...
          child: personasAsync.when(
            ...
            data: (personas) {
              if (personas.isEmpty) {
                return const Center(child: Text('No se encontraron personas disponibles.'));
              }
              return ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  final isSelected = persona.codPersona == _selectedPersona?.codPersona;
                  ...
                  return ListTile(
                    ...
                    title: Text('${persona.datoPersona}', style: context.bodyStyle.copyWith(fontWeight: FontWeight.bold)),
                    subtitle: Text('CI: ${persona.ciNumero} ${persona.ciExpedido.toUpperCase()}', style: context.bodyLightStyle),
  ...
  Future<void> _seleccionarPersona(PersonaEntity persona, bool isSelected) async {
    if (isSelected) {
  ...
  List<Step> get _steps => [
        ...
        Step(
          ...
          content: _currentStep == 1 ? _buildDatosPersonalesContent() : const SizedBox.shrink(),
        ),
        Step(
          title: Text('Contacto', style: TextStyle(fontSize: context.smallFontSize)),
          isActive: _currentStep >= 2,
          state: _currentStep > 2 ? StepState.complete : StepState.indexed,
          content: _currentStep == 2
              ? Column(
                  children: [
                    DetalleTelefono(codPersona: _selectedPersona?.codPersona ?? 0),
        ...
        Step(
          title: Text('Educación', style: TextStyle(fontSize: context.smallFontSize)),
          isActive: _currentStep >= 3,
          state: _currentStep > 3 ? StepState.complete : StepState.indexed,
          content: _currentStep == 3 ? DetalleEducacion(codEmpleado: 0) : const SizedBox.shrink(),
        ),
        Step(
          title: Text('Formación', style: TextStyle(fontSize: context.smallFontSize)),
          isActive: _currentStep >= 4,
          state: _currentStep > 4 ? StepState.complete : StepState.indexed,
          content: _currentStep == 4 ? DetalleFormacion(codEmpleado: 0) : const SizedBox.shrink(),
        ),
        Step(
          title: Text('Experiencia', style: TextStyle(fontSize: context.smallFontSize)),
          isActive: _currentStep >= 5,
          state: _currentStep > 5 ? StepState.complete : StepState.indexed,
          content: _currentStep == 5 ? DetalleExperienciaLaboral(codEmpleado: 0) : const SizedBox.shrink(),
        ),
        Step(
          title: Text('Laboral', style: TextStyle(fontSize: context.smallFontSize)),
          isActive: _currentStep >= 6,
          state: _currentStep > 6 ? StepState.complete : StepState.indexed,
          content: _currentStep == 6
              ? Column(
                  children: [
                    DetalleAreaCargo(),
                    const Divider(height: 32),
        ...
        content: _currentStep == 7 && _selectedPersona != null
    ...
    return Scaffold(
      ...
      body: Center(
        child: Container(
          ...
          child: Stepper(
            type: context.isMobile ? StepperType.vertical : StepperType.horizontal,
          ...
          children: [
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                ...
                foregroundColor: Colors.white,
                padding: EdgeInsets.symmetric(horizontal: context.spacing * 2, vertical: 12),
              ),
              child: Text(_currentStep == _steps.length - 1 ? 'Finalizar Registro' : 'Continuar'),
            ),
          ],
        ),
      ),
...
Widget _buildDatosPersonalesContent() {
  ...
  if (tempPersona != null && tempPersona.codPersona == _selectedPersona!.codPersona) {
    return FormPersona(
  ...
  Future<void> _onStepContinue() async {
    ...
    if (_currentStep == 6) {
      ...
      final tieneRelacion = ref.read(tempRelacionLaboralListProvider).isNotEmpty;
      ...
      if (!tieneRelacion) {
        _showErrorSnackBar('Debe completar la información de la Relación Laboral');
        return;
      }
      setState(() => _currentStep++);
      _saveProgressToLocal();
    ...
    final exito = await executeABM(
      ...
      operation: () async {
        ...
        final empleadoEntityBase = EmpleadoEntity(
          ...
          nombres: '', apPaterno: '', apMaterno: '', ciExpedido: '',
          ciFechaVencimiento: DateTime.now(), ciNumero: '', direccion: '',
          estadoCivil: '', fechaNacimiento: DateTime.now(), lugarNacimiento: '',
          nacionalidad: 0, sexo: '', lat: 0, lng: 0, audUsuarioI: 0,
          datoPersona: '', codEmpleado: 0, numCuenta: '', codRelBeneficios: 0,
          codRelPlanilla: 0, codDependiente: 0, esActivoString: '',
          persona: PersonaEntity.vacio(), // Entidad base para el constructor
          empleadoCargo: EmpleadoCargoEntity(
            codCargoSucursal: 0, codCargoSucPlanilla: 0, fechaInicio: DateTime.now(),
            cargoSucursal: CargoSucursalEntity(codCargoSucursal: 0, codSucursal: 0, codCargo: 0, audUsuario: 0, datoCargo: ''),
            cargoPlanilla: '', existe: 0, audUsuario: 0, codEmpleado: 0,
          ),
          dependiente: null,
          empresa: EmpresaEntity(codEmpresa: 0, nombre: '', codPadre: 0, sigla: '', audUsuario: 0),
          sucursal: SucursalEntity(codSucursal: 0, nombre: '', codEmpresa: 0, codCiudad: 0, audUsuarioI: 0, nombreCiudad: '', codSucursalPlanilla: 0, nombrePlanilla: '', empresa: EmpresaEntity(codEmpresa: 0, nombre: '', codPadre: 0, sigla: '', audUsuario: 0)),
          relEmpEmpr: RelacionLaboralEntity(codRelEmplEmpr: 0, codEmpleado: 0, esActivo: 1, tipoRel: '', nombreFileContrato: '', fechaIni: DateTime.now(), motivoFin: '', audUsuario: 0, cargo: '', sucursal: '', empresaFiscal: '', empresaInterna: ''),
        );
        ...
        final empleadoEntity = empleadoEntityBase.copyWith(
          ...
          empleadoCargo: EmpleadoCargoEntity(codCargoSucursal: 0, codCargoSucPlanilla: 0, fechaInicio: DateTime.now(), cargoSucursal: CargoSucursalEntity(codCargoSucursal: 0, codSucursal: 0, codCargo: 0, audUsuario: 0, datoCargo: ''), cargoPlanilla: '', existe: 0, audUsuario: 0, codEmpleado: 0),
          empresa: EmpresaEntity(codEmpresa: 0, nombre: '', codPadre: 0, sigla: '', audUsuario: 0),
          sucursal: SucursalEntity(codSucursal: 0, nombre: '', codEmpresa: 0, codCiudad: 0, audUsuarioI: 0, nombreCiudad: '', codSucursalPlanilla: 0, nombrePlanilla: '', empresa: EmpresaEntity(codEmpresa: 0, nombre: '', codPadre: 0, sigla: '', audUsuario: 0)),
          relEmpEmpr: RelacionLaboralEntity(codRelEmplEmpr: 0, codEmpleado: 0, esActivo: 1, tipoRel: '', nombreFileContrato: '', fechaIni: DateTime.now(), motivoFin: '', audUsuario: 0, cargo: '', sucursal: '', empresaFiscal: '', empresaInterna: ''),
        );
...
final relacionPrimera = ref.read(tempRelacionLaboralListProvider).firstOrNull;
...
for (var area in areaCargoList) {
  ...
  final cargoSucEntity = area['cargoSucursal'] is CargoSucursalEntity
      ? area['cargoSucursal'] as CargoSucursalEntity
      : (area['cargoSucursal'] is Map 
          ? CargoSucursalModel.fromJson(Map<String, dynamic>.from(area['cargoSucursal'] as Map)).toEntity()
          : null);
...
final relaciones = ref.read(tempRelacionLaboralListProvider);
debugPrint('📋 [4] Total de relaciones a registrar: ${relaciones.length}');
...
for (var i = 0; i < relaciones.length; i++) {
  ...
  try {
    ...
    await ref.read(registrarRelacionLaboral(rel.copyWith(codEmpleado: codEmpleado, audUsuario: codUsuario)).future);
    ...
    try {
      final ultimaRelacion = await ref.read(obtenerUltimaRelacionLaboralProvider(codEmpleado).future);
      codRelEmplEmpr = ultimaRelacion.codRelEmplEmpr;
      debugPrint('✅ [4.${i + 1}] codRelEmplEmpr obtenido: $codRelEmplEmpr');
    } catch (eInner) {
      debugPrint('⚠️ [4.${i + 1}] Error al obtener codRelEmplEmpr: $eInner');
    }
  } catch (e) {
...
if (codRelEmplEmpr > 0) {
  debugPrint('➡️ [4.2] Actualizando Empleado con codRelEmplEmpr=$codRelEmplEmpr');
  try {
    ...
    debugPrint('✅ [4.2] Empleado actualizado correctamente con codRelEmplEmpr=$codRelEmplEmpr');
  } catch (e) {
    debugPrint('❌ [4.2] Error actualizando empleado: $e');
  }
} else {
  debugPrint('⚠️ [4.2] codRelEmplEmpr no se obtuvo (=0), no se actualiza empleado');
}
...
debugPrint('➡️ [5] Iniciando registro de Educación...');
        ...
        for (var edu in educaciones) {
          await ref.read(registrarEducacionProvider(edu.copyWith(codEmpleado: codEmpleado, audUsuario: codUsuario)).future);
        }
        ...
        for (var frm in formaciones) {
          await ref.read(registrarFormacionProvider(frm.copyWith(codEmpleado: codEmpleado, audUsuario: codUsuario)).future);
        }

        // Experiencia
        final experiencias = ref.read(tempExperienciaListProvider);
        for (var exp in experiencias) {
          await ref.read(registrarExperienciaLaboralProvider(exp.copyWith(codEmpleado: codEmpleado, audUsuario: codUsuario)).future);
        }

        // Teléfonos y Emails (vinculados a codPersona)
        final telefonos = ref.read(tempTelefonoListProvider);
        for (var tel in telefonos) {
          await ref.read(registrarTelefonoProvider(tel.copyWith(codPersona: codPersona, audUsuario: codUsuario)).future);
        }
        final emails = ref.read(tempEmailListProvider);
        for (var mail in emails) {
          await ref.read(registrarEmailProvider(mail.copyWith(codPersona: codPersona, audUsuario: codUsuario)).future);
        }
