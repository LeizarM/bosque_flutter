...
class ChipTigoScreen extends ConsumerStatefulWidget { // 1. CAMBIO A STATEFUL
...
actions: [
  IconButton(
    ...
    onPressed: (state.periodoFiltro == null)
        ...
        : () async {
            ...
            await mostrarReportePdf(
              ...
              downloadFunction: () async {
                ...
                return await ref.read(rptPerdidaLineasProvider(state.periodoFiltro!).future);
              },
            );
          },
  ),
      ...
      body: BosqueFlatTable<ChipTigoEntity>(
        ...
        onSearch: (val) => notifier.setSearch(val), // El SQL hace el trabajo pesado
        footer: (state.chipsPerdidos.isNotEmpty && !state.cargando)
            ? SafeArea(child: _buildPaginador(state, notifier))
            : null,
        extraFilters: [
      ...
      items: state.periodos.map((p) {
  return DropdownMenuItem(
    value: p, 
    child: Text(p), // Muestra 'TODOS' o '2026-03' tal cual vienen del SP
  );
}).toList(),
      onChanged: (val) {
        ...
        columns: [
          BosqueColumn(
            ...
            cellBuilder: (e) => Text(e.fila.toString(), style: const TextStyle(fontWeight: FontWeight.w500)),
          ),
          BosqueColumn(
            label: 'Empleado',
            flex: 2,
            cellBuilder: (e) => Text(e.nombreCompleto, style: const TextStyle(fontWeight: FontWeight.w500)),
          ),
          BosqueColumn(
            label: 'Teléfono',
            flex: 1,
  ...
  flex: 1,
  cellBuilder: (e) => DisplayValue<TipoRenovacionChipTigoEntity>(
    code: e.descripcion, // Aquí entra el 'PERD' que manda SQL
    provider: obtenerTipoRenovacionChip, // El provider que trae la lista de v_tipos
    getCode: (item) => item.codTipos, 
    getDescription: (item) => item.nombre, // Esto es lo que se pintará en pantalla
    fallback: e.descripcion, // Si falla, muestra el código por si acaso
...
cellBuilder: (e) => Text(FechaUtils.formatDate(e.fechaSolicitud), style: const TextStyle(fontSize: 12, color: Colors.grey)),//Text(DateFormat('yyyy-MM-dd').format(e.fechaSolicitud)
),
          BosqueColumn(
            label: 'Acciones',
            flex: 1, 
  ...
  child: ListTile(
    title: Text(e.nombreCompleto, style: const TextStyle(fontWeight: FontWeight.bold)),
    subtitle: Column(
      ...
      children: [
        ...
        Text('Fecha: ${FechaUtils.formatDate(e.fechaSolicitud)}', style: const TextStyle(fontSize: 12, color: Colors.grey)),
        if (e.codigo != null && e.codigo!.isNotEmpty)
          Text('Código: ${e.codigo}', style: const TextStyle(fontSize: 12, color: Colors.blueGrey)),
      ],
    ),
    trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                ...
                IconButton(
                  icon: Icon(Icons.delete_outline, color: state.guardando ? Colors.grey : Colors.red),
                  onPressed: state.guardando ? null : () => _confirmarEliminacion(context, ref, e),
                ),
              ],
            ),
...
void _confirmarEliminacion(BuildContext context, WidgetRef ref, ChipTigoEntity entity) async {
  final confirmar = await showDialog<bool>(
    ...
    builder: (ctx) => AlertDialog(
      ...
      content: Text('¿Está seguro de eliminar el registro de ${entity.nombreCompleto}?\nEsta acción no se puede deshacer.'),
      actions: [
        TextButton(
...
// 5. MÉTODO DEL PAGINADOR (Asegúrate de que use los métodos del notifier)
  Widget _buildPaginador(dynamic state, dynamic notifier) {
    return Container(
      color: Colors.white,
      ...
      child: Row(
        ...
        children: [
          IconButton(
            ...
            onPressed: state.pagina > 1 ? () => notifier.setPagina(state.pagina - 1) : null,
          ),
          Text('Página ${state.pagina}'),
          IconButton(
            icon: const Icon(Icons.chevron_right),
            onPressed: state.chipsPerdidos.length >= state.tamanoPagina 
                ? () => notifier.setPagina(state.pagina + 1) 
                : null,
          ),
        ],
      ),
