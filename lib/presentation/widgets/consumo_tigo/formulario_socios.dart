...
class FormularioSocioState extends ConsumerState<FormularioSocios> {
  ...
  void _handleSubmit() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        final socios =
            widget.isEditing && widget.socios != null
        ...
        if (context.mounted) {
          if (widget.isEditing) {
            ...
            AppSnackbarCustom.showAdd(context, 'Socio agregado correctamente');
          }
  ...
  Widget _buildSocios() {
    ...
    return sociosTigo.when(
      data:
          (tigoSocios) => DropdownButtonFormField<int>(
            isExpanded: true,
            value:
                tigoSocios.any(
                      (asociado) => asociado.codEmpleado == _socioSeleccionado,
                    )
                    ? _socioSeleccionado
                    : null,
  ...
  Widget _buildNombreCompletoInput() {
    return TextFormField(
      ...
      readOnly:
          _socioSeleccionado != null, // Solo lectura si hay socio seleccionado
    );
  }

  Widget _buildTelefonoInput() {
    final nroSinAsignarAsync = ref.watch(
      obtenerNroSinAsignar(widget.periodoCobrado),
    );
                    ...
                    suffixIcon:
                        nros.isNotEmpty
                            ? IconButton(
                              ...
                              onPressed:
                                  () => _mostrarDialogoNumerosSinAsignar(nros),
                            )
                            : null,
                  ),
                  keyboardType: TextInputType.number,
                  validator:
                      (value) => validarSoloNumeros(value, esObligatorio: true),
                ),
              ],
            ),
            if (nros.isNotEmpty) _buildRecordatorioNumerosSinAsignar(nros),
          ],
        );
      },
    );
  ...
  Future<void> _mostrarDialogoNumerosSinAsignar(
    List<SocioTigoEntity> nros,
  ) async {
  ...
  Widget _buildButtons(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        ...
        SizedBox(
          width: ResponsiveUtilsBosque.getHorizontalPadding(context) / 2,
        ),
        ElevatedButton.icon(
          onPressed: _handleSubmit,
          icon: Icon(
            widget.isEditing ? Icons.save : Icons.check,
            color: Colors.white,
          ),
          label: Text(widget.isEditing ? 'Actualizar' : 'Guardar'),
          style: ElevatedButton.styleFrom(
            backgroundColor:
                widget.isEditing ? Colors.orange[800] : Colors.blue[800],
            padding: EdgeInsets.symmetric(
              horizontal:
                  ResponsiveUtilsBosque.getHorizontalPadding(context) / 2,
              vertical: 14,
