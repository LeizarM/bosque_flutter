...
class FormularioZonaState extends ConsumerState<FormularioZona> {
    ...
    if (widget.isEditing && widget.zona != null) {
      ...
      Future.microtask(() async {
        ...
        for (final pais in paises) {
          ...
          CiudadEntity? ciudad;
          try {
            ciudad = ciudades.firstWhere(
              (c) => c.codCiudad == widget.codCiudad,
            );
          } catch (_) {
            ciudad = null;
          }
  ...
  void _handleSubmit() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        final zona =
            widget.isEditing && widget.zona != null
        ...
        if (context.mounted) {
          if (widget.isEditing) {
            ...
            AppSnackbarCustom.showAdd(context, 'Zona agregado correctamente');
          }
  ...
  Widget _buildPaisDropdown() {
    ...
    return paisesAsync.when(
      data:
          (paises) => DropdownButtonFormField<int>(
  ...
  Widget _buildCiudadDropdown() {
    ...
    return ciudadesAsync.when(
      data:
          (ciudades) => DropdownButtonFormField<int>(
            ...
            validator:
                (value) => value == null ? 'Seleccione una ciudad' : null,
          ),
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (error, _) => Text('Error: $error'),
    );
