...
class FormularioPersonaState extends ConsumerState<FormularioPersona> {
  ...
  void _initControllers() {
    _controllers = {
      ...
      'ciFechaVencimiento': TextEditingController(
        text:
            widget.persona?.ciFechaVencimiento != null
                ? FormatearFecha.formatearFecha(
                  widget.persona!.ciFechaVencimiento!,
                )
                : '',
      ),
      'fechaNacimiento': TextEditingController(
        text:
            widget.persona?.fechaNacimiento != null
                ? FormatearFecha.formatearFecha(
  ...
  void _validateCiOnType(String ciNumero) {
    ...
    _debounce = Timer(const Duration(milliseconds: 700), () async {
      try {
        final PersonaEntity personaExistente = await ref.read(
        ...
        if (errorString.contains('409') ||
          ...
          if (mounted) {
            setState(() {
              _ciErrorMessage =
                  '❌ Este C.I. ya pertenece a otra persona. No se puede usar.';
            });
          }
        }
  ...
  void loadPersona(PersonaEntity persona) {
    ...
    setState(() {
      ...
      _ciudadSeleccionada =
          persona
              .codZona; // Asumo que codZona también tiene el codCiudad asociado si lo necesitas para repopular la ciudad
  ...
  Future<PersonaEntity> getPersona() async {
    ...
    final codPersonaFinal =
        widget.isEditing
            ? widget.persona?.codPersona ?? _personaTemp.codPersona
            : _personaTemp
                .codPersona; // Si fue autocompletado, _personaTemp.codPersona != 0
    return PersonaEntity(
      ...
      ciFechaVencimiento: FormatearFecha.parseFecha(
        _controllers['ciFechaVencimiento']!.text,
      ),
      fechaNacimiento: FormatearFecha.parseFecha(
        _controllers['fechaNacimiento']!.text,
      ),
    ...
    Widget formWidget = Form(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(
            height: 8,
          ), // Espacio para que los campos no queden tapados por los botones
        ],
      ),
    );
  ...
  List<Widget> _buildDatosAdicionalesFields(bool isLargeScreen) {
    return [
      ...
      Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Expanded(child: _buildNacionalidadField(flex: isLargeScreen ? 1 : 1)),
      ...
      if (_nacionalidadSeleccionado != null)
        Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Expanded(child: _buildCiudadField(flex: isLargeScreen ? 1 : 1)),
            const SizedBox(width: 8),
            IconButton(
              icon: const Icon(Icons.add_circle_outline),
                  ...
                  builder:
                      (ctx) => AlertDialog(
                        content: FormularioCiudad(
                          ...
                          onSave: (ciudad) async {
                            await ref.read(
                              registrarCiudadProvider(ciudad).future,
                            );
                            ref.invalidate(
                              ciudadProvider(_nacionalidadSeleccionado!),
                            );
                          },
                          onCancel: () => Navigator.of(ctx).pop(),
                        ),
      ...
      if (_ciudadSeleccionada != null)
        Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Expanded(child: _buildZonaField(flex: isLargeScreen ? 2 : 1)),
            const SizedBox(width: 8),
            IconButton(
              icon: const Icon(Icons.add_circle_outline),
  ...
  Widget _buildNacionalidadField({required int flex}) {
    return _buildDropdownField<int>(
      ...
      validator: (value) => validarDropdown(value?.toString(), 'nacionalidad'),
      flex: flex,
    );
  }
  ...
  Widget _buildCiudadField({required int flex}) {
    ...
    final ciudadesSeleccionadas = ref
        .watch(ciudadProvider(nacionalidad!))
        .when(
    ...
    if (nacionalidad != 1) {
      final ciudadesBolivia = ref
          .watch(ciudadProvider(1))
          .when(
            data: (ciudades) => ciudades,
            loading: () => <dynamic>[],
            error: (_, __) => <dynamic>[],
      ...
      ciudadesFinal = [
        ...
        ...ciudadesBolivia.where(
          (ciudadB) =>
              !ciudadesSeleccionadas.any(
                (c) => c.codCiudad == ciudadB.codCiudad,
              ),
        ),
      ];
    }
    ...
    return _buildDropdownField<int>(
      ...
      validator: (value) => validarDropdown(value?.toString(), 'ciudad'),
      flex: flex,
    );
  }
  ...
  Widget _buildZonaField({required int flex}) {
    return ConstrainedBox(
      ...
      child: DropdownSearch<ZonaEntity>(
        selectedItem: ref
            .watch(zonaProvider(_ciudadSeleccionada!))
            .whenOrNull(
              data:
                  (zonas) => zonas.firstWhere(
