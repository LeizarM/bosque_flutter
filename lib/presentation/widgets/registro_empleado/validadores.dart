...
String? validarFechaFinRelacion(String? value, String fechaInicioText) {
  if (value == null || value.isEmpty) {
    return 'Requerido';
  }
...
Future<void> validarHistorialRelacion({
  ...
  console(
    '   Rango:  [${FechaUtils.formatDate(fIniNueva)}, ${FechaUtils.formatDate(fFinNueva)}]',
  );
  ...
  final historial = await ref.refresh(
    getHistorialRelLabEmpleado(codEmpleado).future,
  );
  ...
  if (historial.isEmpty) {
    console(
      '⚠️  El historial está vacío. Validación completada sin conflictos.',
    );
    return;
  }
  ...
  for (int i = 0; i < historial.length; i++) {
    ...
    final fFin =
        rel.fechaFin != null
            ...
            : 'SIN FIN (presente)';
    console(
      '   [$i] ID ${rel.codRelEmplEmpr} $estado: ${FechaUtils.formatDate(rel.fechaIni!)} - $fFin',
    );
  }
  ...
  try {
    ...
    console(
      '   Fin:    ${relacionActiva.fechaFin != null ? FechaUtils.formatDate(relacionActiva.fechaFin!) : "SIN FIN (PRESENTE)"}',
    );
  } catch (e) {
  ...
  for (final rel in historial) {
    ...
    console('   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console(
      '   📌 COMPARANDO CON: ID ${rel.codRelEmplEmpr} (${rel.esActivo == 1 ? "ACTIVO" : "INACTIVO"})',
    );
    console(
      '      Rango: [${FechaUtils.formatDate(fIniEx)}, ${fFinEx != null ? FechaUtils.formatDate(fFinEx) : "PRESENTE"}]',
    );
    ...
    if (rel.esActivo == 1) {
      ...
      console(
        '          Comparando: ${FechaUtils.formatDate(fFinNueva)} < ${FechaUtils.formatDate(fIniEx)} ?',
      );

      if (fFinNueva.isAfter(fIniEx)) {
        console(
          '          ❌ FAIL: Fin nueva (${FechaUtils.formatDate(fFinNueva)}) > Inicio activa (${FechaUtils.formatDate(fIniEx)})',
        );
        console(
          '      CONFLICTO: El historial se solapa con la relación activa',
        );
        throw CustomError(
      ...
      if (fFinNueva.isAtSameMomentAs(fIniEx)) {
        console(
          '          ❌ FAIL: Fin nueva (${FechaUtils.formatDate(fFinNueva)}) = Inicio activa (${FechaUtils.formatDate(fIniEx)}) [misma fecha no permitida]',
        );
        throw CustomError(
      ...
      console(
        '          ✅ PASS: ${FechaUtils.formatDate(fFinNueva)} < ${FechaUtils.formatDate(fIniEx)}',
      );
    }
    ...
    if (rel.esActivo == 0 && fFinEx != null) {
      ...
      console(
        '          Nuevo:     [${FechaUtils.formatDate(fIniNueva)}, ${FechaUtils.formatDate(fFinNueva)}]',
      );
      console(
        '          Existente: [${FechaUtils.formatDate(fIniEx)}, ${FechaUtils.formatDate(fFinEx)}]',
      );
      ...
      final traslapeInicio =
          fIniNueva.isBefore(fFinEx) || fIniNueva.isAtSameMomentAs(fFinEx);
      final traslapeFin =
          fFinNueva.isAfter(fIniEx) || fFinNueva.isAtSameMomentAs(fIniEx);
      final hayTraslape = traslapeInicio && traslapeFin;

      console(
        '          traslapeInicio ($fIniNueva <= $fFinEx): $traslapeInicio',
      );
...
Future<void> validarEdicionRelacionActiva({
  ...
  try {
    ...
    if (ultimaInactiva == null) {
      console(
        '⚠️  NO HAY relaciones inactivas con fechaFin. Validación completada.',
      );
      return;
    }
    ...
    console(
      '   ¿${FechaUtils.formatDate(fIniNueva)} > ${FechaUtils.formatDate(fFinUltima)}? ',
    );

    if (fIniNueva.isBefore(fFinUltima) ||
        fIniNueva.isAtSameMomentAs(fFinUltima)) {
...
Future<void> validarFechaRelacionYCargo({
  ...
  final fRelNormalizada = DateTime(
    fechaRelacion.year,
    fechaRelacion.month,
    fechaRelacion.day,
  );
  final fCargoNormalizada = DateTime(
    fechaCargo.year,
    fechaCargo.month,
    fechaCargo.day,
  );
  ...
  if (historialRelaciones.isNotEmpty) {
    ...
    if (ultimaRelInactiva != null) {
      ...
      console(
        '   Última relación inactiva fin: ${FechaUtils.formatDate(fFinRelacion)}',
      );
      console(
        '   ¿${FechaUtils.formatDate(fRelNormalizada)} > ${FechaUtils.formatDate(fFinRelacion)}?',
      );

      if (fRelNormalizada.isBefore(fFinRelacion) ||
          fRelNormalizada.isAtSameMomentAs(fFinRelacion)) {
  ...
  if (historialCargos.isNotEmpty) {
    ...
    if (ultimoCargo.fechaInicio != null) {
      ...
      console(
        '   ¿${FechaUtils.formatDate(fCargoNormalizada)} > ${FechaUtils.formatDate(fUltimoCargo)}?',
      );

      if (fCargoNormalizada.isBefore(fUltimoCargo) ||
          fCargoNormalizada.isAtSameMomentAs(fUltimoCargo)) {
...
Future<void> validarCronologiaCargoUltimoRegistro({
  ...
  console(
    '📋 NUEVA FECHA DE CARGO: ${FechaUtils.formatDate(fCargoNormalizada)}',
  );

  // Detectar si es edición o agregar
  final isEditing = fechaInicioOriginal != null;
  final fCargoOriginalNormalizada =
      isEditing
          ? DateTime(
            fechaInicioOriginal.year,
            fechaInicioOriginal.month,
            fechaInicioOriginal.day,
          )
          : null;

  if (isEditing) {
    console(
      '   Modo: EDICIÓN (Fecha original: ${FechaUtils.formatDate(fCargoOriginalNormalizada!)})',
    );
  } else {
  ...
  try {
    ...
    if (ultimoCargo.fechaInicio != null) {
      ...
      if (isEditing &&
          fUltimoCargo.isAtSameMomentAs(fCargoOriginalNormalizada!)) {
        console('');
        console(
          '⏭️  SKIP: Es el mismo cargo que estamos editando. Buscando el anterior...',
        );

        if (historialCargos.length < 2) {
          console(
            '⚠️  No hay cargo anterior. Validación completada sin conflictos.',
          );
          return;
        }
        ...
        console(
          '   Cargo anterior encontrado: ${FechaUtils.formatDate(fCargoAnterior)}',
        );
        console('');
        console('⚖️  COMPARACIÓN (contra anterior):');
        console(
          '   Nueva fecha:   ${FechaUtils.formatDate(fCargoNormalizada)}',
        );
        console('   Anterior:      ${FechaUtils.formatDate(fCargoAnterior)}');
        console(
          '   ¿${FechaUtils.formatDate(fCargoNormalizada)} > ${FechaUtils.formatDate(fCargoAnterior)}?',
        );

        if (fCargoNormalizada.isBefore(fCargoAnterior) ||
            fCargoNormalizada.isAtSameMomentAs(fCargoAnterior)) {
...
String? validarLugarNacimiento(String? value) {
  ...
  if (!reg.hasMatch(trimmed))
    return 'Lugar de nacimiento contiene caracteres inválidos';
  return null;
}
  ...
  try {
    ...
    if (existente.codTelefono != 0) {
      if (codTelefonoEdicion == null) {
        throw CustomError('El teléfono $numeroTelefono ya está registrado.');
...
///
/// Retorna: mensaje de error o null si es válido
String? validarFechaAfiliacion(String? value, {String? fechaBajaText}) {
  if (value == null || value.isEmpty) {
...
///
/// Retorna: mensaje de error o null si es válido
String? validarFechaBaja(String? value, {required String fechaAfiliacionText}) {
