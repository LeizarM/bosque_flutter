...
class BosqueFlatTable<T> extends StatelessWidget {
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          decoration: BoxDecoration(
            color: index.isEven ? Colors.white : Colors.blueGrey[50]!.withOpacity(0.3),
            border: Border(
  ...
  Widget _buildCell(BosqueColumn<T> col, {required bool isHeader, T? item, bool isLast = false}) {
    return Expanded(
      flex: col.flex,
      child: Container(
  ...
  Widget _buildTopBar(BuildContext context, bool isDesktop) {
    return Container(
      ...
      child: isDesktop 
            ...
            children: [
              ...
              if (extraFilters != null)
                Row(
                  ...
                  children: extraFilters!.map((f) => Padding(
        ...
        : Column( // En móvil se mantiene en columna
            children: [
...
class BosqueTreeTable<T> extends StatefulWidget {
  ...
  final String Function(T) idMapper; // Para identificar cada fila y su estado de expansión
  ...
  final BoxDecoration Function(T item, int index, int nivel)? rowDecorationBuilder;
  final bool cargando;
...
class _TreeRow<T> {
  final T item;
  final int nivel;
  final int originalIndex; // Para alternar colores (par/impar) basado en el padre
...
class _BosqueTreeTableState<T> extends State<BosqueTreeTable<T>> {
    ...
    for (int i = 0; i < widget.items.length; i++) {
      ...
      if (_expandedMap[id] == true) {
        ...
        for (var child in children) {
          result.add(_TreeRow(child, 1, i)); // Hijos mantienen el índice del padre para el color base si se desea
        }
      }
    }
    return result;
          ...
          child: Row(
            children: [
              ...
              ...widget.columns.asMap().entries.map((entry) {
                return _buildHeaderCell(entry.value, isLast: entry.key == widget.columns.length - 1);
              }),
            ],
          ),
        ),
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final decoration = widget.rowDecorationBuilder != null 
                  ? widget.rowDecorationBuilder!(item, rowData.originalIndex, nivel)
                  : BoxDecoration(
                      color: rowData.originalIndex % 2 == 0 ? Colors.white : Colors.grey[100],
            ...
            return Container(
                ...
                child: Row(
                  children: [
                    ...
                    // Celdas de datos
                    ...widget.columns.asMap().entries.map((entry) {
                      return _buildDataCell(entry.value, item, nivel, isLast: entry.key == widget.columns.length - 1);
                    }),
                  ],
                ),
              );
  ...
  Widget _buildDataCell(BosqueTreeColumn<T> col, T item, int nivel, {bool isLast = false}) {
    return Expanded(
      flex: col.flex,
      child: Container(
        ...
        alignment: col.alignment,
        decoration: BoxDecoration(
          border: isLast ? null : Border(right: BorderSide(color: Colors.grey[300]!, width: 1)),
        ),
...
extension SnackbarListener on WidgetRef {
  void listenMessages(ProviderListenable<dynamic> provider, BuildContext context) {
    listen<dynamic>(provider, (previous, next) {
      ...
      } catch (_) {} // Falla en silencio si el módulo no tiene la variable 'mensajeExito'
      ...
      try {
        if (next.mensajeAdvertencia != null && next.mensajeAdvertencia.isNotEmpty) {
          if (previous == null || previous.mensajeAdvertencia != next.mensajeAdvertencia) {
            _showCustomSnackBar(context, next.mensajeAdvertencia, Colors.orange);
          }
        }
