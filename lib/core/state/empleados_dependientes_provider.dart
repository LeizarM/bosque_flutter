...
final empleadosDependientesProvider =
...
final empleadoProvider = FutureProvider.family<EmpleadoEntity, int>((
  ref,
  codEmpleado,
) async {
  final empleados = await ref.watch(
    empleadosDependientesProvider(codEmpleado).future,
  );
...
final dependientesProvider =
    FutureProvider.family<List<DependienteEntity>, int>((
      ref,
      codEmpleado,
    ) async {
      final repo = FichaTrabajadorImpl();
...
final estadoCivilProvider = FutureProvider<List<EstadoCivilEntity>>((
  ref,
) async {
  final repo = FichaTrabajadorImpl();
...
final zonaProvider = FutureProvider.family<List<ZonaEntity>, int>((
  ref,
  codCiudad,
) async {
  final repo = FichaTrabajadorImpl();
...
final ciudadProvider = FutureProvider.family<List<CiudadEntity>, int>((
  ref,
  codPais,
) async {
  final repo = FichaTrabajadorImpl();
...
final editarDependienteProvider =
    StateProvider.family<Future<List<DependienteEntity>>, DependienteEntity>(
      (ref, dependiente) => ref
          .read(dependientesNotifierProvider.notifier)
          .editarDependiente(dependiente),
    );
final registrarPersonaProvider =
...
final editarDepProvider =
    FutureProvider.family<List<DependienteEntity>, DependienteEntity>((
      ref,
      dependiente,
    ) async {
      final repo = FichaTrabajadorImpl();
...
final telefonoProvider = FutureProvider.family<List<TelefonoEntity>, int>((
  ref,
  codPersona,
) async {
  final repo = FichaTrabajadorImpl();
...
final tipoTelefonoProvider = FutureProvider<List<TipoTelefonoEntity>>((
  ref,
) async {
  final repo = FichaTrabajadorImpl();
...
final registrarTelefonoProvider =
    FutureProvider.family<List<TelefonoEntity>, TelefonoEntity>((
      ref,
      telefono,
    ) async {
      final repo = FichaTrabajadorImpl();
...
final registrarEmailProvider =
    FutureProvider.family<List<EmailEntity>, EmailEntity>((ref, email) async {
      final repo = FichaTrabajadorImpl();
      return await repo.registrarEmail(email);
    });
final emailProvider = FutureProvider.family<List<EmailEntity>, int>((
  ref,
  codPersona,
) async {
  final repo = FichaTrabajadorImpl();
...
final formacionProvider = FutureProvider.family<List<FormacionEntity>, int>((
  ref,
  codEmpleado,
) async {
  final repo = FichaTrabajadorImpl();
...
final registrarFormacionProvider =
    FutureProvider.family<List<FormacionEntity>, FormacionEntity>((
      ref,
      formacion,
    ) async {
      final repo = FichaTrabajadorImpl();
...
final obtenerTipoFormacionProvider = FutureProvider<List<TipoFormacionEntity>>((
  ref,
) async {
  final repo = FichaTrabajadorImpl();
...
final obtenerTipoDuracionFormacionProvider =
    FutureProvider<List<TipoDuracionFormacionEntity>>((ref) async {
      final repo = FichaTrabajadorImpl();
...
final experienciaLaboralProvider = FutureProvider.family<
  List<ExperienciaLaboralEntity>,
  int
>((ref, codEmpleado) async {
  final repo = FichaTrabajadorImpl();
  final experienciaLaboral = await repo.obtenerExperienciaLaboral(codEmpleado);
  return experienciaLaboral;
});
final registrarExperienciaLaboralProvider = FutureProvider.family<
  List<ExperienciaLaboralEntity>,
...
final obtenerTipoGaranteReferenciaProvider =
    FutureProvider<List<TipoGaranteReferenciaEntity>>((ref) async {
      final repo = FichaTrabajadorImpl();
      final tipoGaranteReferencia = await repo.obtenerTipoGaranteRef();
      return tipoGaranteReferencia;
    });
final obtenerGaranteReferenciaProvider = FutureProvider.family<
  List<GaranteReferenciaEntity>,
  int
>((ref, codEmpleado) async {
  final repo = FichaTrabajadorImpl();
...
final registrarGaranteReferenciaProvider = FutureProvider.family<
  List<GaranteReferenciaEntity>,
...
final empObtenerDatosEmpleado = FutureProvider.family<EmpleadoEntity, int>((
  ref,
  codEmpleado,
) async {
  final repo = FichaTrabajadorImpl();
...
//providers para manejar datos del empleado
final empObtenerDatosEmpleados = FutureProvider.family<int, int>((
  ref,
  codEmpleado,
) async {
  console(
    '⭐ Provider empObtenerDatosEmpleados - Iniciando con codEmpleado: $codEmpleado',
  );
  final repo = FichaTrabajadorImpl();
  final empleado = await repo.obtenerDatosEmp(codEmpleado);

  console(
    '⭐ Provider empObtenerDatosEmpleados - Respuesta del repo: $empleado',
  );

  if (empleado.isEmpty) {
    console('❌ Provider empObtenerDatosEmpleados - Lista vacía');
    throw Exception('No se encontró el empleado');
  }

  final codPersona = empleado.first.codPersona;
  console(
    '⭐ Provider empObtenerDatosEmpleados - codPersona obtenido: $codPersona',
  );
...
final subirFotoDocProvider =
    FutureProvider.family<bool, (int, String, Uint8List, String)>((
      ref,
      params,
    ) async {
      final repository = FichaTrabajadorImpl();
...
final registrarZonaProvider =
    FutureProvider.family<List<ZonaEntity>, ZonaEntity>((ref, zona) async {
      final repo = FichaTrabajadorImpl();
      return await repo.registrarZona(zona);
    });
final registrarCiudadProvider =
    FutureProvider.family<List<CiudadEntity>, CiudadEntity>((
      ref,
      ciudad,
    ) async {
      final repo = FichaTrabajadorImpl();
...
final registrarPaisProvider =
    FutureProvider.family<List<PaisEntity>, PaisEntity>((ref, pais) async {
      final repo = FichaTrabajadorImpl();
      return await repo.registrarPais(pais);
    });
final todosLosDocumentosProvider =
    FutureProvider.family<Map<String, List<String>>, int>((
      ref,
      codPersona,
    ) async {
      final repository = FichaTrabajadorImpl();
...
final documentosPendientesProvider = FutureProvider<List<Map<String, dynamic>>>(
  (ref) async {
    final repo = FichaTrabajadorImpl();
...
class WarningCounterNotifier extends StateNotifier<int> {
  ...
  WarningCounterNotifier(this.codEmpleado, int initialValue)
    : super(initialValue);
...
final warningCounterProvider =
    StateNotifierProvider<WarningCounterNotifier, int>((ref) {
...
//reportes
final jasperPdfProvider = FutureProvider.family<Uint8List, int>((
  ref,
  codEmpleado,
) async {
  final repo = FichaTrabajadorImpl();
...
//bloqueo de usuario
final registrarBloqueoUsuarioProvider =
...
final desbloquearUsuarioProvider = FutureProvider.family<bool, int>((
  ref,
  codUsuario,
) async {
  final repo = FichaTrabajadorImpl();
...
final usuarioBloqueadoProvider =
    FutureProvider.family<UsuarioBloqueadoEntity, int>((ref, codUsuario) async {
      final repo = FichaTrabajadorImpl();
...
final obtenerPersonaXCodPersona = FutureProvider.family<PersonaEntity, int>((
  ref,
  codEmpleado,
) async {
  console(
...
final obtenerCorporativoEmpleado =
    FutureProvider.family<TelefonoEntity, (int, String)>((ref, params) async {
      final repo = FichaTrabajadorImpl();
      final corporativoXEmpleado = await repo.obtenerCorporativoEmpleado(
        params.$1,
        params.$2,
      );
...
final empleadoXJerarquiaProvider =
    FutureProvider.family<EmpleadoEntity, (int, int)>((ref, params) async {
      final repo = FichaTrabajadorImpl();
