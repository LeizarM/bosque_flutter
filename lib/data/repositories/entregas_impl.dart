...
import 'package:bosque_flutter/data/models/pendiente_entrega_model.dart';
...
class EntregasImpl implements EntregasRepository {
  ...
  @override
  Future<List<PendienteEntregaEntity>> getPendientesEntrega() async {
    try {
      final response = await _dio.post(AppConstants.pendientesDeEntrega);

      if (response.statusCode == 200 && response.data != null) {
        final items =
            (response.data as List<dynamic>)
                .map(
                  (json) => PendienteEntregaModel.fromJson(
        ...
        throw Exception('Error al obtener los pendientes de entrega');
      }
    } on DioException catch (e) {
      String errorMessage = 'Error de conexión: ${e.message}';
      if (e.response != null && e.response!.data != null) {
