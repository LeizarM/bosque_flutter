...
class RRHHRepositoryImpl implements RRHHRepository {
  ...
  /// Obtener descuentos (préstamos y multas) de un empleado por mes y año
  Future<List<DescuentoEmpleadoEntity>> obtenerDescuentosEmpleado({
    ...
    try {
      final response = await _dio.post(
        AppConstants.descObtenerDescuentosEmpleado,
        data: {'codEmpleado': codEmpleado, 'mes': mes, 'anio': anio},
      );
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        final items =
            (data as List<dynamic>)
                .map((json) => DescuentoEmpleadoModel.fromJson(json))
                .toList();
