...
@override
Future<List<EmpleadoEntity>>getLstEmpleados(String? search, int? esActivo, int pageNumber, int pageSize,int? codEmpresa)async{
  try{
    final response = await  _dio.post(AppConstants.rrhhObtenerLstEmpleados,
    data:{
      'search':search,
      'esActivo':esActivo,
    ...
    if(response.statusCode ==200 && response.data !=null){
      ...
      final items = ( data as List<dynamic>).map((json)=>EmpleadoModel.fromJson(json)).toList();
...
Future<EmpleadoEntity> registrarEmpleado(EmpleadoEntity empleado) async {
  try {
    ...
    if ((response.statusCode == 201 || response.statusCode == 200) && 
        response.data != null && response.data['ok'] == 'ok') {
      ...
      try {
        ...
        if (response2.statusCode == 200 && response2.data != null) {
            ...
            if (dataMap.containsKey('codEmpleado')) {
              ...
              for (final entry in dataMap.entries) {
                if (entry.value is int && entry.value > 0) {
                  ...
                  console('   ✅ Encontrado como "${entry.key}": $codEmpleado');
                  break;
                }
              }
            }
          ...
          console('❌ Respuesta inválida - statusCode: ${response2.statusCode}');
        }
      } catch (e) {
      ...
      console('⏳ [FALLBACK] Buscando en lista de empleados por codPersona...');
      try {
       ...
       // console('📥 Respuesta de lista: statusCode=${respLst.statusCode}, items=${respLst.data.length if respLst.data is List else 0}');
        ...
        if (respLst.statusCode == 200 && respLst.data is List) {
          final items = (respLst.data as List<dynamic>)
          ...
          for (final emp in items) {
            console('   - codPersona=${emp.codPersona}, codEmpleado=${emp.codEmpleado}');
            if (emp.codPersona == empleado.codPersona && emp.codEmpleado > 0) {
              console('✅ [FALLBACK-OK] Empleado encontrado: codEmpleado=${emp.codEmpleado}');
              return emp.toEntity();
            }
          }
          
          console('❌ No se encontró empleado en lista con codPersona=${empleado.codPersona}');
        }
      } catch (e) {
      ...
      throw Exception('El empleado se registró pero no se pudo obtener su código');
      
    } else {
      console('❌ Error en registro: statusCode=${response.statusCode}');
      console('   Mensaje: ${response.data?['msg'] ?? 'desconocido'}');
      throw Exception('${response.data?['msg'] ?? 'Error desconocido en registro'}');
    }
  ...
  if (e.response != null && e.response?.data != null) {
    ...
    errorServidor = e.response?.data['msg'] ?? "Error desconocido en el servidor";
  }

  throw Exception(errorServidor); // Lanzamos el mensaje limpio: "El sueldo es menor al..."
} catch (e) {
...
Future<List<PersonaEntity>>getLstPersonas(String buscarPersona)async{
  try{
    final response = await  _dio.post(AppConstants.rrhhObtenerLstPersonas,
    data:{
      'buscarPersona':buscarPersona,
    });
    if(response.statusCode ==200 && response.data !=null){
      final data = response.data ?? [];
      final items = ( data as List<dynamic>).map((json)=>PersonaModel.fromJson(json)).toList();
      return items.map((model)=> model.toEntity()).toList();
    }else{
      return [];
    }
...
//OBTENDRA LOS DATOS DE UNA PERSONA SEGUN SU CODPERSONA
 @override
  ...
  Future<List<EducacionEntity>> obtenerEducacion(int codEmpleado)async {
    try{
      final response = await _dio.post(AppConstants.rrhhObtenerEducacion,
...
@override
  Future<List<CargoSucursalEntity>> obtenerCargoXsucursal(int codSucursal)async {
    try {
      final response = await _dio.post(AppConstants.rrhhObtenerCargoXSucursal,
...
@override
  Future<List<NroCuentaBancariaEntity>> getCuentaBancoXEmpleado(int codEmpleado)async {
    try {
      final response = await _dio.post(AppConstants.rrhhGetCuentaBancoXEmpleado,
        data: {'codEmpleado': codEmpleado});
      if (response.statusCode == 200 && response.data != null) {
        final data = response.data ?? [];
        final items = (data as List<dynamic>)
  ...
  @override
Future<NroCuentaBancariaEntity>registrarCuentaBancaria(NroCuentaBancariaEntity cuenta)async{
  try {
...
//REGISTRARA UN NUEVO EMPLEADO
@override
Future<EmpleadoCargoEntity> registroEmpleadoCargo(EmpleadoCargoEntity empleadoCargo) async {
  try {
    ...
    if ((response.statusCode == 201 || response.statusCode == 200) && response.data != null) {
      ...
      if (ok == 'ok' || msg.contains('exitosa') || msg.contains('actualizados')) {
    ...
    final errorMsg = e.response?.data?['msg'] ?? e.message ?? 'Error de conexión';
  ...
  Future<EmpleadoEntity> obtenerDetalleEmpleado(int codEmpleado) async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        throw Exception('Error al obtener detalle empleado: ${response.statusCode}');
      }
    } catch (e) {
...
@override
  Future<EmpleadoEntity> obtenerCargoActual(int codEmpleado) async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        throw Exception('Error al obtener cargo actual del empleado: ${response.statusCode}');
      }
    } catch (e) {
  ...
  @override
  Future<List<EmpleadoEntity>> obtenerHistorialCargosEmpleado(int codEmpleado)async {
    try {
      final response = await _dio.post(AppConstants.rrhhObtenerHistorialCargosEmpleado,
        data: {'codEmpleado': codEmpleado});
      if (response.statusCode == 200 && response.data != null) {
        final data = response.data ?? [];
        final items = (data as List<dynamic>)
  ...
  @override
  Future<List<RelacionLaboralEntity>> obtenerHistorialRelLabEmpleado(int codEmpleado)async {
    try {
      final response = await _dio.post(AppConstants.rrhhObtenerHistorialRelacionLaboral,
        data: {'codEmpleado': codEmpleado});
      if (response.statusCode == 200 && response.data != null) {
        final data = response.data ?? [];
        final items = (data as List<dynamic>)
      ...
      console('Error al obtener relaciones laborales del empleado: ${e.message}');
      return [];
    } catch (e) {
      console('Error al obtener el historial de relaciones laborales del empleado: $e');
      return [];
    }
  }
  //eliminar empleado cargo 
    @override
  Future<bool> eliminarEmpleadoCargo(int codEmpleado,int codCargoSucursal, DateTime fechaIni,int codCargoSucPlanilla)async {
    try {
      ...
      console('📤 URL: ${AppConstants.baseUrl}${AppConstants.rrhhEliminarEmpleadoCargo}/$codEmpleado/$codCargoSucursal/$fechaLimpia/$codCargoSucPlanilla');
      console('==========================================');
...
@override
Future<RelacionLaboralEntity> obtenerUltimaRelacionLaboral(int codEmpleado) async {
  try {
    console('🔍 Obteniendo última relación laboral para codEmpleado: $codEmpleado');
    ...
    if (response.statusCode == 200 && response.data != null) {
      ...
      if (relEntity.codRelEmplEmpr > 0) {
        console('✅ Relación obtenida: codRelEmplEmpr=${relEntity.codRelEmplEmpr}');
        return relEntity;
      } else {
        throw Exception('No se encontró relación laboral registrada');
      }
    } else {
      throw Exception('Error al obtener relación laboral: ${response.statusCode}');
    }
  } on DioException catch (e) {
    console('❌ DioException: ${e.message}');
    throw Exception('Error de conexión: ${e.message}');
...
@override
Future<List<LicenciaConducirEntity>> obtenerLicenciasConducir(int codPersona) async {
  try {
    final response = await _dio.post(
...
@override
Future<LicenciaConducirEntity> registrarLicenciaConducir(LicenciaConducirEntity licencia) async {
...
Future<List<TipoLicenciaEntity>> obtenerTipoLicenciaConducir() async {
  try {
    ...
    if (response.statusCode == 200 && response.data != null) {
      final data = response.data ?? [];
      final items = (data as List<dynamic>).map((json) => TipoLicenciaModel.fromJson(json)).toList();
      return items.map((model) => model.toEntity()).toList();    
    } else {
      return [];
    }
...
@override
Future<bool> eliminarFoto(int codEmpleado, String tipoDocumento, String nombreArchivo) async {
  try {
...
@override
Future<List<EmpleadoEntity>>getCargosXEmpresa(String? search, int? codEmpresa)async{
  try{
    final response = await  _dio.post(AppConstants.cargoXempresa,
    data:{
      'search':search,
      'codEmpresa':codEmpresa,
    });
...
@override
  Future<AfiliacionSeguroEntity?> obtenerAfiliacionSeguro(int codEmpleado) async {
    try {
      final response = await _dio.post(
...
@override
Future<AfiliacionSeguroEntity> registrarAfiliacionSeguro(AfiliacionSeguroEntity afiliacion) async {
  ...
  Future<EmpleadoEntity> obtenerHaberBasico(int codEmpleado) async {
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        throw Exception('Error al obtener haber basico: ${response.statusCode}');
      }
    } catch (e) {
